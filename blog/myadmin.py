...
def activity_delete(request, id):
    ...
    if activity:
        for old_pics in activity[0].activity_images.all():
            old_pics.delete()
        activity.delete()
    return HttpResponseRedirect(request.META.get('HTTP_REFERER', '/'))
...
@login_required(login_url=LOGIN_URL)
def proposal_seconded(request, pid):
    appitem = get_appitem(request.user)
    proposal = appitem.proposal_set.get(id = pid)
    if proposal:
        proposal_seconded = proposal.proposal_seconded.all()
        matchs, show_pages = page_turning(proposal_seconded, request, 20)
        context = {
            'appitem': appitem,
            'proposal': proposal,
            'proposal_seconded': proposal_seconded,
            'matchs': matchs,
            'show_pages': show_pages,
        }
        return render_to_response('yimi_admin/proposal_seconded.html', context,
            context_instance=RequestContext(request))
    return HttpResponseRedirect(request.META.get('HTTP_REFERER', '/'))


@login_required(login_url=LOGIN_URL)
def proposal_vote(request, pid):
    appitem = get_appitem(request.user)
    proposal = appitem.proposal_set.get(id = pid)
    if proposal:
        proposal_vote = proposal.proposal_vote.all()
        matchs, show_pages = page_turning(proposal_vote, request, 20)
        context = {
            'appitem': appitem,
            'proposal': proposal,
        ...
        return render_to_response('yimi_admin/proposal_vote.html', context,
            context_instance=RequestContext(request))
    return HttpResponseRedirect(request.META.get('HTTP_REFERER', '/'))
...
@login_required(login_url=LOGIN_URL)
def proposal_discuss(request, pid):
    appitem = get_appitem(request.user)
    proposal = appitem.proposal_set.get(id = pid)
    if proposal:
        proposal_discuss = proposal.proposal_discuss.all()
        matchs, show_pages = page_turning(proposal_discuss, request, 20)
        context = {
            'appitem': appitem,
            'proposal': proposal,
        ...
        return render_to_response('yimi_admin/proposal_discuss.html', context,
            context_instance=RequestContext(request))
    return HttpResponseRedirect(request.META.get('HTTP_REFERER', '/'))
