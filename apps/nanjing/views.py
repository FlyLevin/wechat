...
def proposal_list(request, slug):
    ...
    open_id = request.GET.get('open_id')
    from django.db.models import Q
    proposals = appitem.proposal_set.filter(~Q(proposal_stage = proposal_stages['PROPOSAL_STAGE_CLOSE'])&~Q(proposal_stage = proposal_stages['PROPOSAL_STAGE_FREEZE'])).all()
